...
Scalar ground_station_visibility(const Scalar& utc_jd, double station_lat) {
    ...
    Scalar season_factor = (Scalar(1.0) + janus::cos(season_angle)) * Scalar(0.5);

    // High-latitude stations have more extreme seasonal variation
    double lat_effect = std::abs(std::sin(station_lat));
    Scalar visibility = night_factor * (Scalar(1.0 - 0.5 * lat_effect) +
 ...
 */
template <typename Scalar>
Scalar satellite_overhead(const Scalar& utc_jd, double orbit_period, double phase_offset) {
    Scalar tai_jd = utc_to_tai_symbolic(utc_jd);
    ...
    Scalar overhead_factor = janus::exp(-dist_from_overhead * dist_from_overhead * 100.0);
 ...
 */
template <typename Scalar>
Scalar mission_objective(const Scalar& mission_start_jd, double mission_duration_days,
                         double station_lat, double orbit_period) {
...
int main() {
    std::cout << "╔════════════════════════════════════════════════════════════╗\n";
    std::cout << "║   Multi-Month Mission Planning with Symbolic Leap Seconds  ║\n";
    std::cout << "╚════════════════════════════════════════════════════════════╝\n\n";
    ...
    std::cout << "  Current TAI-UTC: " << LEAP_SECOND_TABLE.back().delta_at << " seconds\n\n";
    ...
    std::cout << "UTC to TAI conversion (fully symbolic with interpolated leap seconds):\n";
    for (const auto& [y, m, d] : test_dates) {
        double utc_jd = calendar_to_jd(y, m, d, 12, 0, 0.0);
    ...
    auto objective = mission_objective(start_jd, duration, station_lat, orbit_period);
    ...
    opti.subject_to(start_offset <= 365.0 - duration);  // Room for 180-day mission

    std::cout << "Optimizing mission start date for maximum data collection...\n\n";
    ...
    auto [y2, m2, d2, h2, mi2, s2] = jd_to_calendar(optimal_start_jd + duration);

    std::cout << std::fixed << std::setprecision(2);
    std::cout << "Optimal mission window:\n";
    std::cout << "  Start: " << y1 << "-" << std::setw(2) << std::setfill('0') << m1
              << "-" << std::setw(2) << d1 << std::setfill(' ') << "\n";
    std::cout << "  End:   " << y2 << "-" << std::setw(2) << std::setfill('0') << m2
              << "-" << std::setw(2) << d2 << std::setfill(' ') << "\n";
    ...
    janus::Function f_obj("mission_obj", {start_offset},
        {mission_objective(base_jd + start_offset, duration, station_lat, orbit_period)});
    ...
    for (int month = 1; month <= 12; month += 2) {
        ...
        if (offset + duration > 365.0) continue;
    ...
    janus::export_graph_html(delta_at_sym, "graph_leap_seconds", "Leap_Second_Interpolator");
    std::cout << "✓ Exported: graph_leap_seconds.html\n";

    // Show the full UTC to TAI conversion
    janus::export_graph_html(tai_jd_sym, "graph_utc_to_tai", "UTC_to_TAI_Symbolic");
    ...
    janus::export_graph_html(gs_vis_sym, "graph_ground_station", "Ground_Station_Visibility");
    ...
    std::cout << "  to maintain differentiability across the 6-month timespan.\n";
