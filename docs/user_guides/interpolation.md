...
The `vulcan::Table1D`, `vulcan::TableND`, `vulcan::ScatteredTable1D`, and `vulcan::ScatteredTableND` classes provide lookup table interpolation for aerodynamic coefficients, atmospheric properties, and other data. They wrap Metis's interpolators for symbolic compatibility.
...
int n_tests = 20;
metis::NumericMatrix points(n_tests, 2);
metis::NumericVector values(n_tests);
...
All table classes work with `metis::SymbolicScalar` for trajectory optimization:

```cpp
Table1D drag_table(mach_pts, cd_pts);

// Symbolic query
metis::SymbolicScalar mach_sym = metis::sym("mach");
metis::SymbolicScalar cd_sym = drag_table(mach_sym);

// Build differentiable function
metis::Function cd_fn("cd_lookup", {mach_sym}, {cd_sym});
...
auto grad = metis::jacobian(y_sym, x_sym);

metis::Function df("df", {x_sym}, {grad});
...
```cpp
Table1D linear(x, y, metis::InterpolationMethod::Linear);   // C0 (default)
Table1D bspline(x, y, metis::InterpolationMethod::BSpline); // C2 smooth
```
...
- [Metis Interpolation Guide](../../reference/metis/docs/user_guides/interpolation.md)
