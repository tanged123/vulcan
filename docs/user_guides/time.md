...
```cpp
// Uses metis::Interpolator for smooth, differentiable approximation
...
- `metis::Opti` with time constraints
...
metis::export_graph_html(epoch.jd_tt(), "graph_jd_tt", "JD_TT_Conversion");

// Export visibility objective (complex graph)
auto visibility = visibility_metric(epoch, sat_period);
metis::export_graph_html(visibility, "graph_visibility", "Visibility_Objective");
```
...
- **Symbolic leap second interpolation** via `metis::Interpolator`
...
> uses `metis::Interpolator` with linear interpolation for smooth gradients.
