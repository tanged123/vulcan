...
cartesian_to_keplerian(const Vec3<Scalar> &r, const Vec3<Scalar> &v,
    ...
    // Inclination [0, pi]
    oe.i = metis::acos(metis::clamp(h(2) / h_mag, Scalar(-1.0), Scalar(1.0)));

    // RAAN [0, 2pi] - handle equatorial case
    const Scalar n_mag_safe = metis::where(n_mag < 1e-10, Scalar(1.0), n_mag);
    Scalar Omega_temp =
        metis::acos(metis::clamp(n(0) / n_mag_safe, Scalar(-1.0), Scalar(1.0)));
    oe.Omega = metis::where(n(1) < 0.0, 2.0 * M_PI - Omega_temp, Omega_temp);
    oe.Omega = metis::where(n_mag < 1e-10, Scalar(0.0), oe.Omega);

    // Argument of periapsis [0, 2pi] - handle circular/equatorial case
    const Scalar e_safe = metis::where(oe.e < 1e-10, Scalar(1.0), oe.e);
    const Scalar n_dot_e = metis::dot(n, e_vec);
    Scalar omega_temp = metis::acos(metis::clamp(
        n_dot_e / (n_mag_safe * e_safe), Scalar(-1.0), Scalar(1.0)));
    oe.omega =
        metis::where(e_vec(2) < 0.0, 2.0 * M_PI - omega_temp, omega_temp);
    oe.omega = metis::where(oe.e < 1e-10, Scalar(0.0), oe.omega);
    oe.omega = metis::where(n_mag < 1e-10, Scalar(0.0), oe.omega);

    // True anomaly [0, 2pi]
    const Scalar e_dot_r = metis::dot(e_vec, r);
    Scalar nu_temp = metis::acos(
        metis::clamp(e_dot_r / (e_safe * r_mag), Scalar(-1.0), Scalar(1.0)));
    oe.nu = metis::where(r_dot_v < 0.0, 2.0 * M_PI - nu_temp, nu_temp);
    oe.nu = metis::where(oe.e < 1e-10, Scalar(0.0), oe.nu);
