...
Scalar shadow_cylindrical(const Vec3<Scalar> &r_sat, const Vec3<Scalar> &r_sun,
    ...
    const Scalar behind_earth =
        metis::where(proj < 0.0, Scalar(1.0), Scalar(0.0));
    const Scalar in_cylinder =
        metis::where(d_perp < R_body, Scalar(1.0), Scalar(0.0));
...
Scalar shadow_conical(const Vec3<Scalar> &r_sat, const Vec3<Scalar> &r_sun,
    ...
    const Scalar theta_body = metis::asin(R_body / r_sat_mag);
    const Scalar theta_sun = metis::asin(R_sun / s);
    ...
    const Scalar cos_theta_clamped =
        metis::where(cos_theta > 1.0, Scalar(1.0),
                     metis::where(cos_theta < -1.0, Scalar(-1.0), cos_theta));
    const Scalar theta = metis::acos(cos_theta_clamped);
    ...
    const Scalar full_sun =
        metis::where(theta >= sum_angles, Scalar(1.0), Scalar(0.0));

    // Penumbra transition: linear interpolation
    const Scalar in_penumbra_region =
        metis::where(theta > diff_angles, Scalar(1.0), Scalar(0.0)) *
        metis::where(theta < sum_angles, Scalar(1.0), Scalar(0.0));
    ...
    const Scalar full_umbra =
        metis::where(theta <= diff_angles, Scalar(0.0), penumbra_frac);
