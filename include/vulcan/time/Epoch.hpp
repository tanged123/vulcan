...
class Epoch {
  public:
     ...
     * @param delta_at TAI - UTC offset for this epoch (default: 37 for post-2017)
     */
     ...
     *       For symbolic mode, create from numeric calendar then add symbolic duration.
     */
    static Epoch from_utc(int year, int month, int day,
    ...
    static Epoch from_gps_week(int week, const Scalar& seconds_of_week, int delta_at = 37) {
        Scalar gps_sec_since_epoch = static_cast<double>(week) * constants::time::SECONDS_PER_WEEK + seconds_of_week;
        ...
        constexpr double GPS_EPOCH_TAI_SEC = (constants::time::JD_GPS_EPOCH - constants::time::JD_J2000) 
     ...
     */
    [[nodiscard]] const Scalar& tai_seconds() const {
        return m_tai_sec;
    }
    ...
    [[nodiscard]] Scalar gps_seconds_of_week() const {
        ...
        if constexpr (std::is_floating_point_v<Scalar>) {
            double weeks = std::floor(static_cast<double>(days_since_gps_epoch) / 7.0);
            ...
            Scalar days_into_week = janus::fmod(days_since_gps_epoch, Scalar(7.0));
            return days_into_week * constants::time::SECONDS_PER_DAY;
        }
    }
     ...
     */
    [[nodiscard]] std::tuple<int, int, int, int, int, double> to_utc_calendar() const
        requires std::is_floating_point_v<Scalar>
    {
        double utc_jd = jd_utc();
